...
const backgroundImage =
  "https://wallpapercat.com/w/full/d/6/e/2138352-2560x1440-desktop-hd-high-tech-background-photo.jpg"
  ...
  const handleLogin = async () => {
    ...
    if (!email || !password) {
      ...
      if (result.status == 200) {          //success response
        ...
        setTimeout(() => {
          if (result.data.existingUser.email == 'marketifyadmin@gmail.com') {
            navigate('/admin')
          }
          else {
            navigate('/home')
      ...
      >
       <Link to={'/'}>
          <div className="text-violet-700 text-3xl font-bold mb-6 flex justify-center items-center gap-2">
            <ShoppingBag className="w-8 h-8 " />  Login to Marketify
          </div>
       </Link>
